...
class Mavic3PIDControl(BaseControl):
    """PID control class for the DJI Mavic 3 drone (X-config).

    Based on SimplePIDControl with gains scaled for Mavic3 mass and inertia.
    Uses force-based PID with nnlsRPM motor mixing and X-config allocation matrix.
    ...
    def __init__(self,
        ...
        if self.DRONE_MODEL != DroneModel.MAVIC3:
            ...
            exit()
        self.P_COEFF_FOR = np.array([.90, .90, 1.80])
        self.I_COEFF_FOR = np.array([.001, .001, .01])
        self.D_COEFF_FOR = np.array([4.5, 4.5, 7.15])
        self.P_COEFF_TOR = np.array([1.04, 1.20, .018])
        self.I_COEFF_TOR = np.array([0.0, 0.0, 0])
        self.D_COEFF_TOR = np.array([0.090, 0.10, 0])
    ...
    def computeControl(self,
        ...
        if target_rpy[2]!=0:
            print("\n[WARNING] ctrl it", self.control_counter, "in Mavic3PIDControl.computeControl(), desired yaw={:.0f}deg but locked to 0. for DroneModel.MAVIC3".format(target_rpy[2]*(180/np.pi)))
    ...
    def _simplePIDPositionControl(self,
        ...
        self.integral_pos_e[2] = np.clip(self.integral_pos_e[2], -0.5, .5)
    ...
    def _simplePIDAttitudeControl(self,
        ...
        xy_clip = self.MAX_XY_TORQUE * 0.95
        z_clip = self.MAX_Z_TORQUE * 0.95
        target_torques = np.clip(target_torques,
                                 [-xy_clip, -xy_clip, -z_clip],
                                 [xy_clip, xy_clip, z_clip])
