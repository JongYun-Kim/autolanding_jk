...
class DroneModel(Enum):
    ...
    MAVIC3 = "mavic3"  # DJI Mavic 3 (0.895 kg, X config)
...
class BaseAviary(gym.Env):
    ...
    def __init__(self,
        ...
        if self.DRONE_MODEL in [DroneModel.CF2X, DroneModel.MAVIC3]:
            # X-configuration: motors at 45 degrees, torque arm is L/sqrt(2)
            self.MAX_XY_TORQUE = (2*self.L*self.KF*self.MAX_RPM**2)/np.sqrt(2)
        elif self.DRONE_MODEL in [DroneModel.CF2P, DroneModel.HB]:
            # Plus-configuration: motors aligned with axes
        ...
        if self.DYNAMICS_ATTR:
            if self.DRONE_MODEL in [DroneModel.CF2X, DroneModel.MAVIC3]:
                # X-configuration motor mixer matrix
                ...
                # Plus-configuration motor mixer matrix
    ...
    def _get_random_drone_init_pos(self, init_xyzs):
        if init_xyzs is None:
            z_min, z_max = 0.8, 6.0
            # z_min, z_max = 4.0, 10.0
            z_rand = random.uniform(z_min, z_max)

            x_base, y_base = 5.0, 7.0  # Should be same as the gv_pos
            r_max = z_rand * 0.1
            r = random.uniform(0, r_max)
            theta = random.uniform(0, 2*np.pi)
            x_rand = x_base + r * np.cos(theta)
            y_rand = y_base + r * np.sin(theta)

            random_xyzs = np.array([[x_rand, y_rand, z_rand]])
        else:
            d_xyzs = -1.0 + (2.0 * np.random.rand(*init_xyzs.shape))
            random_xyzs = init_xyzs + d_xyzs
    ...
    def _housekeeping(self):
        ...
        for i in range(self.NUM_DRONES):
            # Show the frame of reference of the drone, note that; It SEVERELY SLOWs down the GUI
    ...
    def _gv_steering_calculator(self, v_des, trajectory_shape):
        """
        :param v_des: (float): desired average velocity of the ground vehicle
        :param trajectory_shape: (int): choice of the trajectory shape
        :return: steering: the desired steering angle
        """
    ...
    def _groundEffect(self, rpm, nth_drone):
        ...
        link_states = np.array(data, dtype=object)
