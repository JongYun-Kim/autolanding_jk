...
class DSLPIDControl(BaseControl):
    ...
    def __init__(self,
        ...
        if self.DRONE_MODEL not in [DroneModel.CF2X, DroneModel.CF2P, DroneModel.MAVIC3]:
            print("[ERROR] in DSLPIDControl.__init__(), DSLPIDControl requires DroneModel.CF2X, DroneModel.CF2P, or DroneModel.MAVIC3")
            exit()
        # Default PID coefficients (tuned for Crazyflie, used as starting point for MAVIC3)
        ...
        if self.DRONE_MODEL in [DroneModel.CF2X, DroneModel.MAVIC3]:
            # X configuration mixer matrix (MAVIC3 uses same X layout as CF2X)
